...
from .transform import GeneralizedRCNNTransform,resize_boxes
...
class FasterRCNNBase(nn.Module):
    ...
    def forward(self, images, targets=None):
        ...
        t4=time.time()
        # print('transform.postprocess',t4-t3)




        #
        # # sun's draw proposals--------------------------------------------------------------------------------------
        # index=2
        # image=Image.open(f"./image/{index}.png")
        # draw=ImageDraw.Draw(image)
        # ratios_height = original_image_sizes[0][0] / images.image_sizes[0][0]
        # ratios_width = original_image_sizes[0][1] / images.image_sizes[0][1]
        # for xmin,ymin,xmax,ymax in proposals[0]:
        #     left=xmin*ratios_width
        #     right=xmax*ratios_width
        #     top=ymin*ratios_height
        #     bottom=ymax*ratios_height
        #     draw.line([(left, top), (left, bottom), (right, bottom),
        #                (right, top), (left, top)], width=8, fill='#318fb5')
        # image.save(f'./rpn/{index}_rpn.png')
        # print(f'number of proposals:',len(proposals[0]))
        # print(f"number of resutls:",len(detections[0]["boxes"]))
        # f=open('reccord.txt','a+')
        # f.write(f'{index}:{len(proposals[0])}  {len(detections[0]["boxes"])}\n')
        # f.close()
        # # sun's draw proposals--------------------------------------------------------------------------------------
...
class FasterRCNN(FasterRCNNBase):
    ...
    def __init__(self, backbone, num_classes=None,
        ...
        if rpn_anchor_generator is None:
            anchor_sizes = ((32,), (64,), (128,), (256,), (512,))    # 五个特征预测层
