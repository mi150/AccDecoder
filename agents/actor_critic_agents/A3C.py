...
class A3C(Base_Agent):
    ...
    def __init__(self, config):
        ...
        self.worker_processes = max(1, self.num_processes - 12)
    ...
    def run_n_episodes(self):
        ...
        episode_reward=multiprocessing.Value('d', 0.)
        ...
        self.print_results(episode_number, episode_reward,results_queue)
    ...
    def print_results(self, episode_number, episode_reward,results_queue):
            ...
            if carry_on:
                if not results_queue.empty():
                    with episode_reward.get_lock():
                        if episode_reward.value==0:
                            episode_reward.value=results_queue.get()
                            self.total_episode_score_so_far =results_queue.get()
                        else:
                            episode_reward.value = 0.99 * episode_reward.value + 0.01 * results_queue.get()
                            self.total_episode_score_so_far =0.99*episode_reward.value+0.01* results_queue.get()
        ...
        while True:
            ...
            with self.optimizer_lock:
                ...
                for grads, params in zip(gradients, self.actor_critic.parameters()):
                    params._grad = grads.clone()  # maybe need to do grads.clone()
                self.actor_critic_optimizer.step()
    def locally_save_policy(self):
        """Saves the policy"""
        torch.save(self.actor_critic, "D:/shiyan/server/server/my_dds_sr_619/agents/Models/{}_local_network.pt".format(self.agent_name))
...
class Actor_Critic_Worker(torch.multiprocessing.Process):
    ...
    def run(self):
        """Starts the worker"""
        print("Starts the worker"+str(self.worker_num))
